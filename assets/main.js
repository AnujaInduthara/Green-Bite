...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    .querySelectorAll(".health-tips, .card, .form-block, .results-block")
    .forEach((el) => {
...
const tipDateElement = document.getElementById("tip-date");
if (tipElement && tipDateElement) {
  const now = new Date();
  ...
  tipDateElement.textContent = dateString + ":";
...
function showCustomAlert(message, type = "info") {
  ...
  alertDiv.style.cssText = `
    ...
    right: 20px;
    background: ${
      type === "success" ? "#009C4D" : type === "error" ? "#e74c3c" : "#3498db"
    };
    color: white;
    padding: 15px 20px;
    border-radius: 8px;
