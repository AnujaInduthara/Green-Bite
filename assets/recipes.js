...
fetch("assets/recipes.json")
  ...
  .catch((err) => {
    ...
    if (container)
      container.innerHTML = `<p role="alert">${err.message}. If running locally, please use a local web server.</p>`;
    console.error(err);
  });
...
function renderRecipes(filter = "all", search = "") {
  ...
  filtered.forEach((recipe) => {
    ...
    card.innerHTML = `
      ...
      <button class="view-btn btn-small" data-id="${
        recipe.id
      }">View Recipe</button>
    `;
...
function openModal(id) {
  ...
  body.innerHTML = `
    ...
    <div class="recipe-actions">
      <button class="btn-secondary save-recipe-btn ${
        isSaved ? "saved" : ""
    ...
    recipe.title
  }" class="modal-recipe-image" loading="lazy" decoding="async">
      ...
      <div class="recipe-content-section">
        ...
        <ol class="steps-list">
          ${recipe.steps
            .map(
              (s, index) =>
                `<li><span class="step-number">${index + 1}</span>${s}</li>`
            )
            .join("")}
        </ol>
      </div>
    ...
    <table class="nutrition-table">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>${Math.round(
            (parseInt(recipe.nutrition.protein) / 50) * 100
          )}%</td>
        </tr>
        <tr>
          <td>Carbohydrates</td>
          ...
          <td>${Math.round(
            (parseInt(recipe.nutrition.carbs) / 300) * 100
          )}%</td>
        </tr>
        <tr>
          <td>Fat</td>
